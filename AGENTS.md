...
Official model IDs (e.g. `yolo26n`) are resolved by `lib/core/yolo_model_resolver.dart` to pinned GitHub release assets: Android LiteRT `.tflite` and opt-in QNN `.onnx` from yolo-flutter-app `models-v1.0.0`, and iOS Core ML `.mlpackage.zip` from yolo-ios-app `models-v1.0.0`. These tags are intentionally pinned and duplicated in `scripts/fetch_bundled_models.sh` — keep both in sync when regenerating assets (`scripts/export-tflite-models.py` and `scripts/export-qnn-models.py` for Android; the iOS assets come from the yolo-ios-app repo). Every mobile asset uses a fixed 224 × 224 classification input or 640 × 640 input for every other task.
