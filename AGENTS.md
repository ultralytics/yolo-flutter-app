...
- iOS official model IDs resolve to Core AI `.aimodel.zip` only when the native `isCoreAIAvailable` reply is true (built with the iOS 27 SDK, running iOS 27+, never the simulator); otherwise they resolve to Core ML `.mlpackage.zip`. A bundled asset of either format wins over a download. The plugin uses no Core AI API itself: `.aimodel` loading needs `UltralyticsYOLO >= 8.9.15`, and neither CI nor the simulator can exercise it.
