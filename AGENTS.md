...
Official model IDs (e.g. `yolo26n`) are resolved by `lib/core/yolo_model_resolver.dart` to pinned GitHub release assets: Android LiteRT `.tflite` from yolo-flutter-app `v0.6.6`, opt-in QNN `.onnx` from `v0.3.5`, and iOS Core ML `.mlpackage.zip` from yolo-ios-app `v8.3.0`. These tags are intentionally pinned and duplicated in `scripts/fetch_bundled_models.sh` — keep both in sync when regenerating assets (`scripts/export-tflite-models.py` for Android; the iOS assets come from the yolo-ios-app repo).
