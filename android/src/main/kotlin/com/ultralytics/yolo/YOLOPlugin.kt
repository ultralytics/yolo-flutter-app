...
class YOLOPlugin : FlutterPlugin, ActivityAware, MethodChannel.MethodCallHandler, PluginRegistry.RequestPermissionsResultListener {
  ...
  private fun loadLabels(modelPath: String): List<String> {
    // Prefer embedded metadata (appended ZIP) so Android matches iOS behavior
    YOLOFileUtils.loadLabelsFromAppendedZip(applicationContext, modelPath)?.let {
      Log.d(TAG, "Loaded ${it.size} labels from metadata for $modelPath")
      return it
    }

    Log.w(TAG, "Falling back to COCO labels for $modelPath")
