...
import 'dart:async';
import 'package:flutter/foundation.dart' show defaultTargetPlatform, debugPrint; // Explicitly import debugPrint
...
class YoloViewController {
  ...
  Future<void> _applyThresholds() async {
    ...
    try {
      ...
      debugPrint('YoloViewController: Applied thresholds - confidence: $_confidenceThreshold, IoU: $_iouThreshold, numItems: $_numItemsThreshold');
    } catch (e) {
      ...
      try {
        ...
        await _methodChannel!.invokeMethod('setConfidenceThreshold', {'threshold': _confidenceThreshold});
        debugPrint('YoloViewController: Applied confidence threshold: $_confidenceThreshold');
        await _methodChannel!.invokeMethod('setIoUThreshold', {'threshold': _iouThreshold});
        debugPrint('YoloViewController: Applied IoU threshold: $_iouThreshold');
        await _methodChannel!.invokeMethod('setNumItemsThreshold', {'numItems': _numItemsThreshold});
        debugPrint('YoloViewController: Applied numItems threshold: $_numItemsThreshold');
      } catch (e2) {
  ...
  Future<void> setNumItemsThreshold(int numItems) async {
    ...
    if (_methodChannel == null) {
      debugPrint('YoloViewController: Warning - Cannot apply numItems threshold, view not yet created');
      return;
    }
    try {
      await _methodChannel!.invokeMethod('setNumItemsThreshold', {'numItems': clampedValue});
      debugPrint('YoloViewController: Applied numItems threshold: $_numItemsThreshold');
    } catch (e) {
...
class YoloViewState extends State<YoloView> {
  ...
  void initState() {
    ...
    debugPrint('YoloView (Dart initState): Creating channels with _viewId: $_viewId');
    
    final resultChannelName = 'com.ultralytics.yolo/detectionResults_$_viewId';
    _resultEventChannel = EventChannel(resultChannelName);
    debugPrint('YoloView (Dart initState): Result EventChannel created: $resultChannelName');
    
    final controlChannelName = 'com.ultralytics.yolo/controlChannel_$_viewId';
    _methodChannel = MethodChannel(controlChannelName);
    debugPrint('YoloView (Dart initState): Control MethodChannel created: $controlChannelName');
  ...
  void didUpdateWidget(YoloView oldWidget) {
    ...
    if (oldWidget.onResult != widget.onResult || oldWidget.onPerformanceMetrics != widget.onPerformanceMetrics) {
      if (widget.onResult == null && widget.onPerformanceMetrics == null) {
        _cancelResultSubscription();
      } else { // If at least one callback is now non-null, ensure subscription
        _subscribeToResults(); 
      }
    }
        ...
        if (event is Map) {
          ...
          if (widget.onResult != null && event.containsKey('detections')) {
            try {
              ...
              debugPrint('YoloView: Event keys for detection error: ${event.keys.toList()}');
              if (event.containsKey('detections')) {
                final detections = event['detections'];
                debugPrint('YoloView: Detections type for error: ${detections.runtimeType}');
                if (detections is List && detections.isNotEmpty) {
                  debugPrint('YoloView: First detection keys for error: ${detections.first?.keys?.toList()}');
                }
              }
            }
          }
          ...
          if (widget.onPerformanceMetrics != null) {
            try {
              final double? processingTimeMs = (event['processingTimeMs'] as num?)?.toDouble();
              ...
              if (processingTimeMs != null && fps != null) {
                ...
                debugPrint('YoloView: Called onPerformanceMetrics callback with: processingTimeMs=$processingTimeMs, fps=$fps');
              }
            } catch (e, s) {
              debugPrint('Error parsing performance metrics: $e');
              debugPrint('Stack trace for metrics error: $s');
              debugPrint('YoloView: Event keys for metrics error: ${event.keys.toList()}');
            }
          }
        } else {
          debugPrint('YoloView: Received invalid event format or no relevant callbacks are set. Event type: ${event.runtimeType}');
        }
      },
      onError: (dynamic error, StackTrace stackTrace) { // Added StackTrace
        debugPrint('Error from detection results stream: $error');
        debugPrint('Stack trace from stream error: $stackTrace');
        
        Future.delayed(const Duration(seconds: 2), () {
          if (_resultSubscription != null && mounted) { // Check mounted before resubscribing
            debugPrint('YoloView: Attempting to resubscribe after error');
            _subscribeToResults();
          } else {
            debugPrint('YoloView: Not resubscribing (stream already null or widget disposed)');
          }
        });
      },
      onDone: () {
  ...
  void _cancelResultSubscription() {
    if (_resultSubscription != null) {
      debugPrint('YoloView: Cancelling existing result subscription for $_viewId');
  ...
  Widget build(BuildContext context) {
    ...
    if (defaultTargetPlatform == TargetPlatform.android) {
      ...
      platformView = const Center(child: Text('Platform not supported for YoloView'));
    }
    return platformView;
  }
  
  void _onPlatformViewCreated(int id) {
    debugPrint('YoloView: Platform view created with system id: $id, our viewId: $_viewId');
    
    // _cancelResultSubscription(); // Already called in _subscribeToResults if needed
    
    if (widget.onResult != null || widget.onPerformanceMetrics != null) {
      debugPrint('YoloView: Re-subscribing to results after platform view creation for $_viewId');
      _subscribeToResults();
    }
    
    _effectiveController._init(_methodChannel); // Re-init controller with the now valid method channel
    
    _methodChannel.invokeMethod('setShowUIControls', {'show': widget.showNativeUI});
    
    _methodChannel.setMethodCallHandler((call) async {
      debugPrint('YoloView: Received method call from platform: ${call.method} for $_viewId');
      
      switch (call.method) {
        case 'recreateEventChannel':
          debugPrint('YoloView: Platform requested recreation of event channel for $_viewId');
          _cancelResultSubscription();
          Future.delayed(const Duration(milliseconds: 100), () {
            if (mounted && (widget.onResult != null || widget.onPerformanceMetrics != null)) {
              _subscribeToResults();
