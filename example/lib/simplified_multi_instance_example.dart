...
class SimplifiedMultiInstanceExample extends StatefulWidget {
  ...
  @override
  State<SimplifiedMultiInstanceExample> createState() => _SimplifiedMultiInstanceExampleState();
}

class _SimplifiedMultiInstanceExampleState extends State<SimplifiedMultiInstanceExample> {
...
Classification: Top class ${results[2]['classification']?['topClass'] ?? 'Unknown'}
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Simplified Multi-Instance Example'),
      ),
      body: Padding(
...
Future<void> functionalExample() async {
  ...
  final segmenter = YOLO(modelPath: 'yolov8n-seg.tflite', task: YOLOTask.segment);
...
class DynamicModelExample {
  ...
  Future<Map<String, dynamic>?> runInference(String modelName, Uint8List image) async {
