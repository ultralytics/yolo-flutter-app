...
class _TestAllTasksScreenState extends State<TestAllTasksScreen> {
    ...
    try {
      ...
      if (task != YOLOTask.classify && results['boxes'] != null) {
        ...
        for (int i = 0; i < boxes.length && i < 3; i++) {
          ...
          output.writeln('  Confidence: ${(box['confidence'] * 100).toStringAsFixed(1)}%');
          output.writeln('  Pixel coords: (${box['x1']?.toStringAsFixed(1)}, ${box['y1']?.toStringAsFixed(1)}) to (${box['x2']?.toStringAsFixed(1)}, ${box['y2']?.toStringAsFixed(1)})');
          output.writeln('  Normalized: (${box['x1_norm']?.toStringAsFixed(3)}, ${box['y1_norm']?.toStringAsFixed(3)}) to (${box['x2_norm']?.toStringAsFixed(3)}, ${box['y2_norm']?.toStringAsFixed(3)})\n');
        }
      }
      ...
      switch (task) {
          ...
          if (results['keypoints'] != null) {
            ...
            if (keypoints.isNotEmpty) {
              ...
              for (int i = 0; i < coords.length && i < 3; i++) {
                ...
                output.writeln('    Keypoint ${i + 1}: (${kp['x']?.toStringAsFixed(3)}, ${kp['y']?.toStringAsFixed(3)}) conf: ${kp['confidence']?.toStringAsFixed(2)}');
              }
            }
          }
          break;
        ...
        case YOLOTask.segment:
          if (results['masks'] != null) {
            ...
            if (masks.isNotEmpty) {
              ...
              output.writeln('  Mask 1 size: ${firstMask.length}x${firstMask[0].length}');
            }
          }
          break;
        ...
        case YOLOTask.classify:
          if (results['classification'] != null) {
            ...
            output.writeln('  Confidence: ${(cls['topConfidence'] * 100).toStringAsFixed(1)}%');
            ...
            if (cls['top5Classes'] != null) {
              ...
              for (int i = 0; i < top5.length; i++) {
                output.writeln('    ${i + 1}. ${top5[i]} (${(confs[i] * 100).toStringAsFixed(1)}%)');
              }
            }
          }
          break;
        ...
        case YOLOTask.obb:
          if (results['obb'] != null) {
            ...
            for (int i = 0; i < obbList.length && i < 3; i++) {
              ...
              output.writeln('    Confidence: ${(obb['confidence'] * 100).toStringAsFixed(1)}%');
      ...
      if (results['detections'] != null) {
        ...
        output.writeln('Successfully created ${detections.length} YOLOResult objects');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Test All YOLO Tasks'),
      ),
