...
public class MethodCallHandler: NSObject, VideoCaptureDelegate, InferenceTimeListener, ResultsListener, FpsRateListener {
    ...
    private func loadModel(args: [String: Any], result: @escaping FlutterResult) async {
        ...
        guard let model = args["model"] as? [String: Any],
              ...
              let task = model["task"] as? String else {
            result(flutterError)
            return
        }
    ...
    private func setLensDirection(args: [String: Any], result: @escaping FlutterResult) {
        ...
        if let direction = args["direction"] as? Int {
            print("DEBUG: Switching camera to direction:", direction)
            if let nativeView = self.videoCapture.nativeView {
                print("DEBUG: Found FLNativeView, calling switchCamera")
                nativeView.switchCamera()
                result("Success")
            } else {
                print("DEBUG: Error - No nativeView found")
                result(FlutterError(code: "SWITCH_ERROR", message: "Failed to switch camera", details: nil))
            }
        } else {
    ...
    private func predictOnImage(args: [String: Any], result: @escaping FlutterResult) {
        guard let imagePath = args["imagePath"] as? String,
              let image = try? createCIImage(fromPath: imagePath) else {
