...
public class YOLOView: UIView, VideoCaptureDelegate {
    ...
    private func setupUI() {
        ...
        if showUIControls {
            self.labelSliderNumItems.text =
                "0 items (max " + String(Int(sliderNumItems.value)) + ")"
        }
        ...
        playButton.setImage(
            UIImage(systemName: "play.fill", withConfiguration: config), for: .normal)
        playButton.tintColor = .systemGray
        pauseButton.setImage(
            UIImage(systemName: "pause.fill", withConfiguration: config), for: .normal)
        ...
        switchCameraButton.addTarget(
            self, action: #selector(switchCameraTapped), for: .touchUpInside)
    ...
    public override func layoutSubviews() {
        ...
        if isLandscape {
            ...
            toolbar.frame = CGRect(
                x: 0, y: height - toolBarHeight, width: width, height: toolBarHeight)
    ...
    @objc func pinch(_ pinch: UIPinchGestureRecognizer) {
        ...
        func minMaxZoom(_ factor: CGFloat) -> CGFloat {
            return min(
                min(max(factor, minimumZoom), maximumZoom), device.activeFormat.videoMaxZoomFactor)
        }
...
extension YOLOView: AVCapturePhotoCaptureDelegate {
        ...
        if let dataImage = photo.fileDataRepresentation() {
            ...
            if let orientedCIImage = CIImage(image: image)?.oriented(orientation),
                let cgImage = CIContext().createCGImage(
                    orientedCIImage, from: orientedCIImage.extent)
            {
    ...
    private func convertResultToStreamData(_ result: YOLOResult) -> [String: Any] {
        ...
        // Precompute view transform (image-space -> view-space) for center-crop and mirroring
        let iw = result.orig_shape.width
        let ih = result.orig_shape.height
        let vw = self.bounds.width
        let vh = self.bounds.height
        let hasViewSize = vw > 0 && vh > 0
        let scale = hasViewSize ? max(vw / iw, vh / ih) : 1.0
        let scaledW = iw * scale
        let scaledH = ih * scale
        let dx = hasViewSize ? (vw - scaledW) / 2.0 : 0.0
        let dy = hasViewSize ? (vh - scaledH) / 2.0 : 0.0
        let isFront = self.videoCapture.previewLayer?.connection?.isVideoMirrored ?? false

        func toViewRect(_ imgRect: CGRect) -> [String: Any] {
            var left = imgRect.minX * scale + dx
            var top = imgRect.minY * scale + dy
            var right = imgRect.maxX * scale + dx
            var bottom = imgRect.maxY * scale + dy

            if isFront && hasViewSize {
                let flippedLeft = vw - right
                let flippedRight = vw - left
                left = flippedLeft
                right = flippedRight
            }

            if hasViewSize {
                left = max(0.0, min(left, vw))
                right = max(0.0, min(right, vw))
                top = max(0.0, min(top, vh))
                bottom = max(0.0, min(bottom, vh))
            }
        ...
        if config.includeDetections {
            ...
            if config.includePoses && !result.keypointsList.isEmpty && result.boxes.isEmpty {
                for (poseIndex, keypoints) in result.keypointsList.enumerated() {
                    ...
                    // View-space bounding box (for correct overlay alignment)
                    if hasViewSize {
                        let imgRect = CGRect(
                            x: CGFloat(minX),
                            y: CGFloat(minY),
                        ...
                        detection["viewBoundingBox"] = toViewRect(imgRect)
                    }
            ...
            for (detectionIndex, box) in result.boxes.enumerated() {
                ...
                // View-space bounding box (for correct overlay alignment)
                if hasViewSize {
                    detection["viewBoundingBox"] = toViewRect(box.xywh)
                }
