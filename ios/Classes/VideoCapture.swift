...
func bestCaptureDevice(position: AVCaptureDevice.Position) -> AVCaptureDevice? {
    // Prefer telephoto when enabled
    if UserDefaults.standard.bool(forKey: "use_telephoto"),
        let device = AVCaptureDevice.default(
            .builtInTelephotoCamera, for: .video, position: position)
    {
        return device
    }
    // Try common multi-camera and single camera configurations
    ...
    // Fallback: discover any available camera for the requested position
...
class VideoCapture: NSObject, @unchecked Sendable {
    ...
    private func predictOnFrame(sampleBuffer: CMSampleBuffer) {
        ...
        if currentBuffer == nil, let pixelBuffer = CMSampleBufferGetImageBuffer(sampleBuffer) {
            ...
            predictor.predict(
                sampleBuffer: sampleBuffer, onResultsListener: self, onInferenceTime: self)
            currentBuffer = nil
        }
    }
