...
void main() {
  ...
  group('YOLO Edge Cases Coverage', () {
    ...
    test('predict handles keypoints data correctly for pose estimation', () async {
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMethodCallHandler(channel, (MethodCall methodCall) async {
        if (methodCall.method == 'loadModel') {
          return true;
    ...
    test('switchModel without viewId throws exception', () async {
      ...
      expect(
        ...
        throwsA(isA<YOLOException>()
            .having((e) => e.message, 'message',
                contains('ViewId not set. Make sure this YOLO instance is attached to a YOLOView'))),
      );
    });
  });
  ...
  group('YOLOResult toString Coverage', () {
    test('YOLOResult toString formats correctly', () {
      ...
      expect(str, 'YOLOResult{classIndex: 0, className: person, confidence: 0.95, boundingBox: Rect.fromLTRB(10.0, 20.0, 110.0, 220.0)}');
    });
    ...
    test('YOLOResult toString with different values', () {
      ...
      final str = result.toString();
      expect(str, 'YOLOResult{classIndex: 2, className: car, confidence: 0.87, boundingBox: Rect.fromLTRB(50.0, 75.0, 250.0, 225.0)}');
    });
  });
}