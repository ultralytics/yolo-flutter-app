...
object YOLOFileUtils {
    ...
    fun loadLabelsFromAppendedZip(context: Context, modelPath: String): List<String>? {
        ...
        for (currentPathAttempt in pathsToTry) {
            ...
            // Handle Flutter asset paths - strip flutter_assets/ prefix to get the actual asset path
            if (resolvedPath.startsWith("flutter_assets/")) {
                resolvedPath = resolvedPath.substring("flutter_assets/".length)
            }
            // Also handle paths that might be passed as "assets/models/..." - strip "assets/" prefix
            // since Android's AssetManager expects relative paths from the assets root
