...
class _PoseEstimationScreenState extends State<PoseEstimationScreen> {
  ...
  void initState() {
    super.initState();
    _yolo = YOLO(modelPath: 'yolo11n-pose.tflite', task: YOLOTask.pose);
    _loadModel();
  }
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    ..._poseResults!.asMap().entries.map((
                                      entry,
                                    ) {
  ...
  Widget _buildPoseInfo(int personNumber, YOLOResult pose) {
    ...
    if (pose.keypoints != null && pose.keypointConfidences != null) {
      for (
        int i = 0;
        i < pose.keypoints!.length && i < _keypointNames.length;
        i++
      ) {
...
class PosePainter extends CustomPainter {
    ...
    for (final pose in poseResults) {
      ...
      for (int i = 0; i < skeleton.length; i++) {
        ...
        if (startIdx < pose.keypoints!.length &&
          ...
          final startX = isNormalized
              ? startPoint.x * image.width
              : startPoint.x;
          final startY = isNormalized
              ? startPoint.y * image.height
              : startPoint.y;
          ...
          final limbPaint = Paint()
            ...
            ..strokeWidth =
                6 *
                strokeScale // Scale stroke width
      ...
      for (int i = 0; i < pose.keypoints!.length; i++) {
        if (pose.keypointConfidences![i] > 0.5) {
          ...
          final borderPaint = Paint()
            ...
            ..strokeWidth =
                3 *
                strokeScale // Scale border width
            ..style = PaintingStyle.stroke;
      ...
      final boxPaint = Paint()
        ..color =
            _getDetectionColor(
              0,
            ) // Always use first color (blue) for person class
        ..strokeWidth =
            4 *
            strokeScale // Scale box stroke width
        ..style = PaintingStyle.stroke;
  ...
  bool shouldRepaint(covariant PosePainter oldDelegate) {
    return image != oldDelegate.image || poseResults != oldDelegate.poseResults;
  }
}
