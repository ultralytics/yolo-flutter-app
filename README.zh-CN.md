...
插件会自动下载并缓存当前平台对应的官方产物。可通过 `YOLO.officialModels()` 查看当前平台可用的全部官方 ID。Android TFLite 统一从 Flutter `v0.2.0` release 下载，iOS Core ML 统一从 YOLO iOS `v8.3.0` release 下载，后续包版本发布不会改变模型 URL。
...
官方模型可直接从 `YOLO.defaultOfficialModel()` 或 `YOLO.officialModels()` 开始；自定义模型则直接从你准备实际交付的导出文件开始。 Android TFLite 示例资产来自 [yolo-flutter-app `v0.2.0`](https://github.com/ultralytics/yolo-flutter-app/releases/tag/v0.2.0)， iOS Core ML 示例资产来自 [yolo-ios-app `v8.3.0`](https://github.com/ultralytics/yolo-ios-app/releases/tag/v8.3.0)。
...
在一台有代表性的设备上（三星 Galaxy S26 / Adreno GPU，YOLO26n 检测），GPU 推理约为 每次 7 毫秒，而 CPU 约为 30 毫秒。数值为近似值，会随设备而变化。

int8 和端到端（`nms=True`）模型仍可正常加载和运行——它们的 INT64 算子和 int8 量化无法编译到 GPU，因此会回退到 CPU，而不是报错。fp16 + 非端到端才是最快路径。
