...
- Core AI (`.aimodel`, iOS 27+) keeps the same meaning: `useGpu: true` prefers the Neural Engine and `useGpu: false` runs CPU only. Single-image results report the `accelerator` as `NPU` or `CPU` for Core AI models
