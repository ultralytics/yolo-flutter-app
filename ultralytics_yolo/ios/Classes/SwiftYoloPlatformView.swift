...
public class SwiftYoloPlatformView: NSObject, FlutterPlatformView, FlutterStreamHandler {
    ...
    private func updateThresholds(confidenceThreshold: Double, iouThreshold: Double?) {
        guard let yoloView = yoloView else { return }
        
        print("SwiftYoloPlatformView: Updating thresholds - confidence: \(confidenceThreshold), IoU: \(String(describing: iouThreshold))")
            ...
            case "setThresholds":
                ...
                if let args = call.arguments as? [String: Any],
                   let confidenceThreshold = args["confidenceThreshold"] as? Double {
                    // IoU threshold is optional
                    let iouThreshold = args["iouThreshold"] as? Double
                    
                    print("SwiftYoloPlatformView: Received setThresholds call with confidence: \(confidenceThreshold), IoU: \(String(describing: iouThreshold))")
                    self.updateThresholds(confidenceThreshold: confidenceThreshold, iouThreshold: iouThreshold)
                    result(nil) // Success
                } else {
