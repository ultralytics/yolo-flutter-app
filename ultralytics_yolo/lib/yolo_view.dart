...
class YoloViewController {
  ...
  Future<void> _applyThresholds() async {
    ...
    try {
      ...
      debugPrint(
        'YoloViewController: Applied thresholds - confidence: $_confidenceThreshold, IoU: $_iouThreshold',
      );
    } catch (e) {
...
class YoloView extends StatefulWidget {
  ...
  Future<void> setConfidenceThreshold(double threshold) {
    ...
    throw UnimplementedError(
      'Cannot call on the widget directly - this is implemented by the state',
    );
  }
...
class YoloViewState extends State<YoloView> {
  ...
  void initState() {
    ...
    _resultEventChannel = EventChannel(
      'com.ultralytics.yolo/detectionResults_$_viewId',
    );
        ...
        if (event is Map && widget.onResult != null) {
          try {
            ...
            if (!event.containsKey('detections')) {
              debugPrint(
                'YoloView: Warning - Event missing "detections" key: $event',
              );
              return;
            }
            ...
            if (event is Map) {
              ...
              if (event.containsKey('detections')) {
                ...
                debugPrint(
                  'YoloView: Detections type: ${detections.runtimeType}',
                );
                if (detections is List && detections.isNotEmpty) {
                  debugPrint(
                    'YoloView: First detection keys: ${detections.first?.keys?.toList()}',
                  );
                }
              }
            }
          ...
          debugPrint(
            'YoloView: Received invalid event format or onResult is null',
          );
          debugPrint(
            'YoloView: Event type: ${event.runtimeType}, onResult null: ${widget.onResult == null}',
          );
        }
      },
  ...
  void _onPlatformViewCreated(int id) {
    debugPrint(
      'YoloView: Platform view created with id: $id and viewId: $_viewId',
    );
    ...
    if (widget.onResult != null) {
      debugPrint(
        'YoloView: Re-subscribing to results after platform view creation',
      );
      _subscribeToResults();
    }
    ...
    _methodChannel.setMethodCallHandler((call) async {
      debugPrint(
        'YoloView: Received method call from platform: ${call.method}',
      );

      switch (call.method) {
        case 'recreateEventChannel':
          debugPrint(
            'YoloView: Platform requested recreation of event channel',
          );
