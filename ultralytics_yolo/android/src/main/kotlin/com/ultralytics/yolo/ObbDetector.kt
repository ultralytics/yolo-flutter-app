...
class ObbDetector(
    ...
    override fun predict(bitmap: Bitmap, origWidth: Int, origHeight: Int, rotateForCamera: Boolean): YOLOResult {
        ...
        val inputBuffer = processedImage.buffer  // FloatBuffer

        // === (2) 出力テンソル形状を取得 ===
        val outShape = interpreter.getOutputTensor(0).shape() // 例: [1, outChannels, outAnchors]
        val outBatch = outShape[0]       // 通常1
        val outChannels = outShape[1]    // (4 + numClasses + 1)
        val outAnchors = outShape[2]     // アンカー総数

        val rawOutput = Array(outBatch) {
