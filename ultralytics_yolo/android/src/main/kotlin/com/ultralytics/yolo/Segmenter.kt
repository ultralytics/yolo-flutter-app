...
class Segmenter(
    ...
    init {
        ...
        // 出力2の形状: [1, maskH, maskW, 32] (例: [1,80,80,32])
        // (後続で使用)

        // ImageProcessor の初期化 - both with and without rotation
    ...
    override fun predict(bitmap: Bitmap, origWidth: Int, origHeight: Int, rotateForCamera: Boolean): YOLOResult {
        ...
        // (2) 出力バッファの用意
        val out0Shape = interpreter.getOutputTensor(0).shape() // 例: [1,116,2100]
        val batch0 = out0Shape[0]
        val out0NumFeatures = out0Shape[1]
        val out0NumAnchors = out0Shape[2]

        numClasses = out0NumFeatures - boxFeatureLength - maskConfidenceLength

        val output0 = Array(batch0) { Array(out0NumFeatures) { FloatArray(out0NumAnchors) } }
        val out1Shape = interpreter.getOutputTensor(1).shape() // 例: [1,80,80,32]
        val batch1 = out1Shape[0]
        val maskH = out1Shape[1]
        val maskW = out1Shape[2]
        val maskC = out1Shape[3]
        val output1 = Array(batch1) { Array(maskH) { Array(maskW) { FloatArray(maskC) } } }
