...
// Non-Maximum Suppression (NMS) の実装（降順ソート済みの proposals に対して）
...
Java_com_ultralytics_yolo_ObjectDetector_postprocess(
    ...
    // ボックス候補（proposals）の抽出
    std::vector<DetectedObject> proposals;
    for (int i = 0; i < w; ++i) {
        int class_index = 0;
        float class_score = -FLT_MAX;
        // 各クラスのスコアを取得 (4番目以降がクラススコアと仮定)
        ...
        // スコアが閾値を超えている場合のみ候補に追加
        if (class_score > confidence_threshold) {
            // 中心座標と幅・高さを取得し、左上座標に変換
            float cx = vec[0][i];
            float cy = vec[1][i];
            float w_box = vec[2][i];
            float h_box = vec[3][i];
    ...
    // 結果を2次元配列として返す（各要素：[x, y, width, height, confidence, class_index]）
